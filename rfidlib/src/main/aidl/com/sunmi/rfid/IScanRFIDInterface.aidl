...
interface IScanRFIDInterface {
     ...
     */ 
    void registerReaderCall(in IReaderCall call);
     ...
     */
    void inventory(in byte btRepeat);
     ...
     * @param btRepeat Repeat time of inventory round. When Repeat = 255, The inventory duration is minimized. For example, if the RF field only has one or two tags, the inventory duration could be only 30-50 mS, this function provides a possibility for fast antenna switch applications on multi-ant devices.
     */
    void realTimeInventory(in byte btRepeat);
     ...
     */
    void customizedSessionTargetInventory(in byte btSession,in byte btTarget,in byte btSL,in byte btPhase,in byte btPowerSave,in byte btRepeat);
     ...
     */
    void fastSwitchAntInventory(in byte btA,in byte btStayA,in byte btB,in byte btStayB,
                               in byte btC,in byte btStayC,in byte btD,in byte btStayD,
                               in byte btInterval,in byte btRepeat);
     ...
     */
    void readTag(in byte btMemBank,in byte btWordAdd,in byte btWordCnt,in byte[] btAryPassWord);
     ...
     */
    void writeTag( in byte[] btAryPassWord, in byte btMemBank, in byte btWordAdd, in byte btWordCnt,in byte[] btAryData);
     ...
     */
    void lockTag( in byte[] btAryPassWord, in byte btMemBank, in byte btLockType);
     ...
     */
    void killTag(in byte[] btAryPassWord);
     ...
     */
    void setAccessEpcMatch(in byte btEpcLen, in byte[] btAryEpc);
     ...
     */
    void setImpinjFastTid(in boolean blnOpen,in boolean blnSave);
     ...
     */
    void iso180006BReadTag(in byte[] btAryUID, in byte btWordAdd, in byte btWordCnt);
     ...
     */
    void iso180006BWriteTag( in byte[] btAryUID, in byte btWordAdd, in byte btWordCnt, in byte[] btAryBuffer);
     ...
     */
    void iso180006BLockTag( in byte[] btAryUID, in byte btWordAdd);
     ...
     */
    void iso180006BQueryLockTag( in byte[] btAryUID, in byte btWordAdd);
   ...
   /**
     ...
     */
    void setWorkAntenna(in byte btWorkAntenna);
     ...
     */
    void setOutputAllPower(in byte btOutputPower);
     ...
     */
    void setOutputPower(in byte btPower1, in byte btPower2, in byte btPower3,in byte btPower4);
     ...
     */
    void setFrequencyRegion(in byte btRegion,in byte btStartRegion, byte btEndRegion);
     ...
     */
    void setUserDefineFrequency(in byte btFreqInterval, in byte btChannelQuantity,in int nStartFreq);
     ...
     */
    void writeGpioValue(in byte btChooseGpio,in byte btGpioValue);
     ...
     * @return Succeeded :0, Failed:-1
     */
    void setTemporaryOutputPower(in byte btRfPower);
     ...
     */
    void setReaderIdentifier(in byte[] btAryIdentifier);
     ...
     */
    void setRfLinkProfile(in byte btProfile);
     ...
     * @return Succeeded :0, Failed:-1
     */
    void getRfPortReturnLoss(in byte btFrequency);
   ...
   //----------------SN and Firware Version---------------
     ...
     */
    void setReaderAddress(in byte btNewReadId);
